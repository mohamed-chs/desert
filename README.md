...
- **Check**: `desert check input.ds` (runs Cargo/rustc checks and translates errors)
...
Project source files can import other files using `import "relative/path.ds"`, dotted paths like `import util.math`, or `from "relative/path.ds" import item as alias` / `from util.math import item` (resolved relative to the importing file, `.ds` extension implied). Rust imports are also supported via `import rust.std.cmp.max`, `import rust.std.collections.HashMap as Map`, `from rust.std.cmp import max as maximum, min`, or `import "rust:std::cmp::max"` (currently `std`/`core`/`alloc` roots).
